  ...
  for( auto extents : vec_extents ) {
    ...
    try {
      ...
      for(auto i=0; i<=runs; ++i) {
        ...
        if(i==0) {
          for(auto i=0; i<extents[0]; ++i)
            if(fabs(hdata[i].x/extents[0]-1.0f)>0.00001)
              throw std::runtime_error("Mismatch.");
          continue; //i=0 => warmup
        }
        auto diff = clock::now() - start;
        auto duration = std::chrono::duration<double, std::milli> (diff).count();
        std::cout << "\"cuFFT Standalone\",\"Outplace\",\"Complex\",\"float\",1,\"powerof2\","<<extents[0]<<",0,0,"<<i-1<<",\"Success\",0,0,0,0,0,0,0,0,0,"<<duration<<",0,0,0,"<<ID<<std::endl;
      }
