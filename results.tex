...
\vspace{-1em}
  ...
  \caption{Time-to-solution measured in \textit{gearshifft} (\cufft{}), in a \textit{standalone} \cufft{} application using multipe timer objects and in a standalone application using one timer object (\textit{standalone-tts}) for a single-precision inplace real-to-complex round-trip FFTs on the K80 \cite{taurus}.}
