...
\cref{fig:verify_cufft} indicates a constant, insignificant overhead effect of \gearshifft{}, where \texttt{cufft} refers to the \gearshifft{} implementation, \texttt{cufft-standalone} to the reference code with \gearshifft{} based timer harness and \texttt{cufft-standalone-tts} to the reference code with only one timer for time to solution.
In \cref{fig:tts_verify_a} \texttt{cufft-standalone-tts} shows slightly better performance. For larger FFTs a difference is not noticeable. The average performance can change between different process launches, whose statistical deviations are not examined here.
