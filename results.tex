...
\gearshifft{} is designed to be a lightweight framework with a thin wrapper for the FFT clients. Performance indicators of each benchmark are collected and buffered to be sorted and dumped to a csv file after the last benchmark finished. To verify the impact of \gearshifft{} a configuration file with 1 and with 100 equal extents is used. The \cufft{} standalone code only measures the time to solution for the same roundtrip FFT.
Both invoke a warmup step and 5 repetitions of the benchmark.
The verification code and run scripts can be found online \cite{gearshifft_github}.
\cref{fig:verify_cufft} indicates a constant, almost zero overhead effect of \gearshifft{}.
Even, after 100 benchmarks there is no significant runtime impact visible, which allows reliable measurements with large config files for the FFT extents.

\begin{figure}[!htbp]
\vspace{-1em}
  \centering
  \includegraphics[width=\textwidth]{figures/results_verify_cufft_legend.pdf}\vspace{-1em}
  \subfloat[1024-point FFT]{\includegraphics[width=0.45\textwidth]{figures/results_verify_cufft_a.pdf}\label{fig:tts_verify_a}}
  \hfill
  \subfloat[16777216-point FFT]{\includegraphics[width=0.45\textwidth]{figures/results_verify_cufft_b.pdf}\label{fig:tts_verify_b}}
...
\cref{fig:fftw_plan_flags} compares the time-to-solution to the actual time spent for the FFT operation itself. This illustration makes the cost and the benefit of higher planning flags than \mc{FFTW_ESTIMATE} obvious. \mc{FFTW_MEASURE} imposes a total runtime penalty of 1 to 2 orders of magnitude with respect to \mc{FFTW_ESTIMATE}, but it offers superior performance if limited to pure FFT execution timings compared \mc{FFTW_ESTIMATE}. The wisdom files were generated with \mc{fftw_wisdom} using the flag \mc{-c} to precompute plans for a canonical set of sizes (powers of two and ten up to $2^{20}$) in \mc{FFTW_PATIENT} mode, which spends even more time for finding the optimal plan than \mc{FFTW_MEASURE}. The other flags indicate to use 24 threads and to not reuse system wisdom files:
