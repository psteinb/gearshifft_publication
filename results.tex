...
As power-of-2 shaped input signals do naturally fit the memory hierarchy found on modern CPUs and GPUs, the transformation of non-power-of-2 input signals could be considered a computational challenge. In a lot of conversations, the hypothesis is still communicated very often, that input signals should be padded to power-of-2 shapes in order to achieve the highest possible performance. For this, we evaluated the runtimes of input signals that have dimensions which are powers of $3,5,7$ (referred to as \texttt{radix357}) and to signals with a dimension being a power of 19 (referred to as \texttt{oddshape}). 

\begin{figure}[!tbp]
  \centering
  \includegraphics[width=\textwidth]{figures/results_figure6_legend.pdf}
  \subfloat[Time for FFT]{\includegraphics[width=0.45\textwidth]{figures/results_figure6a.pdf}\label{fig:f6a}}
  \hfill
  \subfloat[Time to Plan]{\includegraphics[width=0.45\textwidth]{figures/results_figure6b.pdf}\label{fig:f6b}}
  \caption{Time for computing single-precision real-to-complex forward transforms using the 3D API \cref{fig:f6a} and the time required to plan the aforementioned execution \cref{fig:f6b}. Both figures use a log10-versus-log2 scale.}
  \label{fig:non_power_of_2}
\end{figure}

Even though our findings in \cref{fig:non_power_of_2} are restricted to FFTW and cuFFT for the sake of simplicity, the figure shows that this ``best practise'' of padding for a power-of-2 can be falsified. As expected, the planning time for non-power-of-2 transforms is longer than for power-of-2 transforms as the multiplicity chunks into smaller high performant tranforms is higher and the performance might be subject to one-off cache line penalties in the memory hierarchy. However, \cref{fig:f6a} emphasizes that the runtime penalty the different kind of transforms is small. This is especially true for FFTW where the non-power-of-2 transforms often touch and overlap with the power-of-2 results. The Cufft library yields higher penalties among the differnt kinds of transforms. Here, the runtime difference between \texttt{oddshape} and \texttt{powerof2} configurations can amount up to one order of magnitude.
