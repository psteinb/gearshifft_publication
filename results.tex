...
As a result, \cref{fig:verify_cufft} indicates a constant, insignificant overhead effect of \gearshifft{}, where:% \texttt{cufft} refers to the \gearshifft{} implementation, \texttt{cufft-standalone} to the reference code with timers for the operations from \cref{fig:framework} and \texttt{cufft-standalone-tts} to the reference code with only one timer for time to solution.
\begin{itemize}
