...
\begin{frame}{Fields of the Fast Fourier Transformation}
  ...
  \begin{tikzpicture}
    \begin{scope}[every node/.style={align=center, anchor=north, font=\bfseries}, xscale=1.15, yscale=2.6]
      ...
      \node[visible on=<3->] at (-3.5, 0.0) {\includegraphics[width=0.25\textwidth]{jpeg-compression.jpg}\\ compression};
...
%level 1 concept/.append style={sibling angle=20}, small mindmap]
    ...
    node[concept,font=\large\bfseries] {Fast Fourier Transforms}
    [grow=down]
...
\begin{table}[tbp]
  ...
  \begin{tabular}{llll}
    \toprule
                        & \textbf{Taurus}           & \textbf{Hypnos}         & \textbf{Islay}                                  \\
                        & HPC Cluster               & HPC Cluster             & Workstation                                     \\
    \midrule
    \textbf{CPU family} & Haswell Xeon               & Haswell Xeon           & Haswell Xeon           \\
    \textbf{CPU model } & $2{\times}$ E5-2680 v3     & $2{\times}$ E5-2603 v3 & $2{\times}$ E5-2640 v3 \\
    ...
    \textbf{GPU driver} & $367.48$                   & $367.48$               & $367.57$               \\
    \textbf{OS}         & RHEL $6.8$                 & Ubuntu $14.04.3$       & CentOS $7.2$           \\
    \bottomrule
  \end{tabular}}}
\end{table}
...
\begin{frame}{Summary}{}
  \begin{itemize}
  \item in-depth analysis of FFT libraries
...
\begin{frame}{Summary Cont.}{}
  ...
  \begin{itemize}
  \item plan is computed once and reused thousands of times
  \item single-core delivers sufficient performance
  \end{itemize}
