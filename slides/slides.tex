...
\begin{frame}{gearshifft}
  \begin{itemize}
  \item \gearshifft{} provides cmake-based C++14 framework
  \item benchmarks operations of a round-trip FFT including memory transfers and context management
  \item various conditions of FFTs can be benchmarked (shapes, precision, transform variants, \ldots)
  \item compare different FFT libraries (cuFFT, clFFT, fftw)
  \item can be extended to further libraries
  \item development on github: \url{https://github.com/mpicbg-scicomp/gearshifft}
  \end{itemize}
  \vfill
  \footnotesize{slides: \url{https://github.com/mpicbg-scicomp/gearshifft_publication/}}
\end{frame}
...
\node[visible on=<4>] (c) at (7.5,0){
  \textbf{+ Results}\\[.5em]
     \begin{lstlisting}[numbers=none,emph={PlanInit},emphstyle={\itshape}]
// ...
timer_start();
init_plan();
val = timer_stop();
...
\item workflow is templated by \mc{FFT.hpp}, implemented by cuFFT, fftw, \ldots
\end{itemize}}
