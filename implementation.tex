...
There are common interfaces for the context management and for the FFT workflow. It requires the user to implement the context and FFT client class. The context methods \mc{create} and \mc{destroy} of the client encapsulate time-consuming device and library initialization, if e.g. the client does not store plan information as \fftw{} does. These methods are measured separately and run only once. 
...
The back-end of \gearshifft{} uses Boost Unit Test Framework to generate the benchmark instances within a tree data structure which is referred to as the benchmark tree.
