...
The \gearshifft{} API documentation can be found online \cite{gearshifft_github}, but we want to discuss the most relevant features here. The front-end interface requires the user to implement the context class and an FFT client class. The context methods \mc{create} and \mc{destroy} of the client encapsulate time-consuming device and library initialization, if e.g. the client does not store plan information as \fftw{} does. These methods are measured separately and run only once. 
